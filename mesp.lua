...
-- loading libraries
local ESP = loadstring(game:HttpGet("https://pastebin.com/raw/BBRGusqA"))()
...
FOVCircle.Radius = tonumber(aimfov)
...
main:NewToggle("AntiAim", "makes localplayer spin so fast", function(state)
    if state then
        aaimenabled = true
...
main:NewTextBox("AntiAim speed", "Speed.", function(txt)
    power = tonumber(txt)
end)
caraaimenabled = false
main:NewToggle("Car AntiAim", "makes car spin so fast", function(state)
    if state then
        caraaimenabled = true
        game:GetService('RunService').Stepped:connect(function()
            game.Players.LocalPlayer.Character.Head.CanCollide = false
        ...
        local bt = Instance.new("BodyThrust")
        bt.Parent = game.Players.LocalPlayer.Character.HumanoidRootPart
        bt.Force = Vector3.new(power1,10,power1)
        bt.Location = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
    else
        caraaimenabled = false
        game.Players.LocalPlayer.Character.HumanoidRootPart.BodyThrust:Remove()
...
main:NewTextBox("Car AntiAim speed", "Speed.", function(txt)
    power1 = tonumber(txt)
end)
main:NewToggle("Specate", "Spectator mode.", function(state)
    if state then
